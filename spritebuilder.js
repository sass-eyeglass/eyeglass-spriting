...
var getSpritesData = function(imageFiles, packingStyle, cb) {
	var imageFileRegexp = /\.(gif|jpg|jpeg|png)$/i;

	var aux = function(spritesData, array, index, prevImage, prevCoordinates, spritemapDimensions, packingStyle, cb) {
		if (index < array.length) {
			lwip.open(array[index], function(err, image) {
				...
				image.toBuffer(encodingFormat, function(err, buffer) {
					...
					aux(spritesData, array, index + 1, image, curCoordinates, spritemapDimensions, packingStyle, cb); 
				});
			});
		}
	...
	aux({}, imageFiles, 0, null, [0, 0], [0, 0], packingStyle, function(spritesData, spritemapDimensions) { return spritesData; }); 
}

// create sprite map png with given filename, and paste in images according to coordinates in spritesData 
...
// stringify spritesData and save it as a json file with given filename 
...
var buildSprites = function(imagesFolder, packingStyle) {
	...
	// var data = getSpritesData(imageFiles, packingStyle); 
	// console.log(data);

	getSpritesData(imageFiles, packingStyle, function(spritesData, spritemapDimensions) {
